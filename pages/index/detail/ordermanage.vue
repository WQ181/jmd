				...
				<view class="row" v-for="(row,index) in list" :key="index">
					...
					<view class="btns">
						...
						<block v-if="row.type=='back'">
							<view class="default" @tap="remindDeliver(row)">确认发货</view>
						</block>
						<!-- <block v-if="row.type=='unreceived'">
							<view class="pay" @tap="confirmReceipt(row)">确认收货</view>
						</block> -->
						<block v-if="row.type=='evalute'">
							<view class="default" @click="toEvaluate(row)">查看客户评价</view>
						</block>
...
<script>
	export default {
		...
		methods: {
			...
			toEvaluate() {
				uni.navigateTo({
					url: '../commodityEvaluate/commodityEvaluate'
				})
			},
			showType(tbIndex) {
